...
const TIMELINE_IDS = ["caral", "chavin", "nazcaMoche", "wariChimu"] as const;
const CULTURE_IDS = [
  "caral",
  "chavin",
  "paracas",
...
const SITE_IDS = ["caral", "paracas", "nazca", "museoLarco"] as const;
const TIP_IDS = ["ideaStory1", "ideaStory2", "curiosity1", "curiosity2"] as const;

export default function PreIncaSection() {
  const t = useTranslations("History.preinca");

  return (
    <section
      id="preinca"
      className="bg-amber-50/60 py-16 scroll-mt-24 md:scroll-mt-28"
    >
      <div className="mx-auto max-w-6xl space-y-10 px-4">
        ...
        <header className="text-center">
          ...
          <p className="mt-4 text-sm text-zinc-700 md:text-base max-w-3xl mx-auto">
            {t("subtitle")}
          </p>

          <div className="mt-4 flex flex-wrap justify-center gap-3 text-xs md:text-sm">
            <span className="rounded-full bg-white px-3 py-1 font-medium text-amber-800 shadow-sm">
              {t("tags.period")}
            </span>
            <span className="rounded-full bg-white px-3 py-1 font-medium text-amber-800 shadow-sm">
              {t("tags.focus")}
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm md:p-7">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-4">
            {TIMELINE_IDS.map((id) => (
              <article
                key={id}
                className="rounded-xl bg-amber-50/60 p-4 text-sm text-zinc-800"
              >
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm md:p-7">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            {CULTURE_IDS.map((id) => (
              <article
                key={id}
                className="flex flex-col gap-1 rounded-xl bg-amber-50/60 p-4 text-sm"
              >
                <h4 className="font-semibold text-zinc-900">
                  {t(`cultures.items.${id}.name`)}
                </h4>
                <p className="text-xs text-amber-900/80 md:text-sm">
                  {t(`cultures.items.${id}.region`)}
                </p>
                <p className="mt-1 text-xs text-zinc-700 md:text-sm">
                  {t(`cultures.items.${id}.description`)}
                </p>
              </article>
            ))}
          </div>
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm md:p-7">
          ...
          <div className="mt-4 grid gap-6 md:grid-cols-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">
                {t("life.religion.title")}
              </p>
              <p className="mt-2 text-sm text-zinc-700 md:text-base">
                {t("life.religion.text")}
              </p>
            </div>
          </div>
        </section>

        {/* BLOQUE 5: LO QUE PUEDES VER HOY */}
        <section className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm md:p-7">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-amber-700">
            {t("sites.title")}
          </h3>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {SITE_IDS.map((id) => (
              <article
                key={id}
                className="flex flex-col justify-between gap-2 rounded-xl bg-amber-50/60 p-4 text-sm"
              >
                <div>
                  <h4 className="font-semibold text-zinc-900">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {t(`sites.items.${id}.mapUrl`) && (
                    <Link
                      ...
                      className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-amber-800 shadow-sm transition hover:bg-amber-100 md:text-sm"
                    >
                  ...
                  {t(`sites.items.${id}.destLink`, { default: "" }) && (
                    <Link
                      ...
                      className="inline-flex items-center gap-2 rounded-full border border-amber-300 bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-800 transition hover:border-amber-500 hover:bg-amber-100 md:text-sm"
                    >
